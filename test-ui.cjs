...
module.exports=function app(saved,failWrites=false,legacy=false){
 ...
 const cx=new Proxy({createLinearGradient:()=>({addColorStop(){}}),drawImage(image,...rest){if(!image)throw Error('Missing image');finite(...rest);}}, {get:(t,p)=>p in t?t[p]:finite,set:(t,p,v)=>(t[p]=v,true)});
 function el(id){return elems[id]??={id,style:{},dataset:{},value:id==='volume'?'30':id==='stageSelect'?'random':id.endsWith('Unit')?'1':'',checked:['sfx','music','motion'].includes(id),open:false,textContent:'',src:'',children:[],classList:{add(){},remove(){},toggle(){}},listeners:{},attributes:{},
  setAttribute(k,v){this.attributes[k]=v;},appendChild(v){this.children.push(v);},querySelector(){return el(id+'-img');},addEventListener(type,fn){this.listeners[type]=fn;},getBoundingClientRect:()=>({left:0,top:0,width:900,height:700}),getContext:()=>cx,toDataURL:()=>('data:image/png;base64,adapter-'+id),setPointerCapture(){},showModal(){this.open=true;},close(){this.open=false;}};}
 ...
 vm.createContext(box);for(const file of['world.js','audio.js','game.js'])vm.runInContext(fs.readFileSync(__dirname+'/dist/'+file,'utf8'),box);
 return{el,storage,registered,box,doc,buttons,events,windowEvents,
  ...
  get(){return E.decode(storage.get('sand-digger.save.v2')||storage.get('sand-digger.save.v1'));},
  submit({duration='77',turn='13',stage='random',seed='test-setting',unit='1',turnUnit='1'}={}){el('duration').value=duration;el('durationUnit').value=unit;el('turnDuration').value=turn;el('turnUnit').value=turnUnit;el('stageSelect').value=stage;el('seedInput').value=seed;el('setupForm').onsubmit({preventDefault(){}});}
 };
};
