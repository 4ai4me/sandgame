...
let currentStage=null,flagImage=null,flagDesign=null,cursors={},lastUI=0,surfaceTexture=null,toolSprites={},toolMotion=null;
...
function rebuild(){
 ...
 for(let y=0;y<E.H;y++)for(let z=0;z<E.N;z++)for(let x=0;x<E.N;x++){
  ...
  for(const f of faceSpec){if(f.n.reduce((a,v,i)=>a+v*view[i],0)<=0)continue;
   ...
   faces.push({texture:{...surfaceTexture,shade:f.shade},poly:f.v.map((v,i)=>({...project(x+v[0],y+v[1]*level,z+v[2]),u:[0,0,1,1][i],v:[0,1,1,0][i]})),d:project(x+.5,y+.5,z+.5).d,color:'rgb('+(E.is3(state)?E.material(state).color:currentStage.sand).map(v=>Math.round((v+grain)*f.shade)).join(',')+')',grain});
  }
 }
 raster.clear();for(const f of [...sceneFaces,...faces])raster.polygon(f.poly,f.color,f.texture);raster.cache();dirty=false;
}
function drawFlag(now){
 ...
 const env=E.environment(state);
 for(let i=0;i<12;i++)for(let row=0;row<3;row++){
  const verts=[[i/12,row/3],[(i+1)/12,row/3],[(i+1)/12,(row+1)/3],[i/12,(row+1)/3]].map(([u,v])=>({...project(...SandFeedback.flagPoint(f,env,now/1000,u,v,reduced)),u,v}));
...
function draw(now){
 ...
 if(toolMotion&&phase==='dig'&&!drag?.rotating){const t=(now-toolMotion.start)/Math.max(180,E.TOOLS[toolMotion.tool].interval);if(t<=1)SandFeedback.drawTool(ctx,toolSprites[toolMotion.tool],toolMotion.tool,toolMotion.x,toolMotion.y,t,reduced);}
 const seasons={spring:'봄',summer:'여름',autumn:'가을',winter:'겨울'},weather={clear:'맑음',rain:'비',snow:'눈',wind:'강풍'};$('environmentLabel').textContent=seasons[env.season]+' · '+(env.daylight>.4?'낮':'밤')+' · '+weather[env.weather]+' · 바람 '+Math.round(env.windStrength*100)+'% '+Math.round((env.windDirection*180/Math.PI+360)%360)+'°';
}
function setScene(){
 surfaceTexture=SandFeedback.surface(E.material(state));currentStage=C.STAGES.find(s=>s.id===state.stage);sceneBoxes=W.build(currentStage);const f=E.factor(state);for(const b of sceneBoxes){b.x=12.5+(b.x-12.5)*f;b.z=12.5+(b.z-12.5)*f;if(b.w>20)b.w*=f;if(b.d>20)b.d*=f;}sceneBoxes.push({x:12.5-14*f,y:-.3,z:12.5-14*f,w:28*f,h:.3,d:28*f,color:'#d5c397'});audio.setTrack(currentStage.track);dirty=true;
...
function updateClock(){
 ...
 $('turnClock').textContent=!state.turnStarted?'0.001g 채굴 후 시작':state.turnDuration?Math.ceil(state.turnRemaining)+'초':'무제한';
}
function updateUI(){
 ...
 $('harvest').textContent=E.harvestGrams(state,harvest).toFixed(6).replace(/0+$/,'').replace(/\.$/,'')+' g';$('pass').disabled=phase!=='observe'||!!state.result||!!drag||!state.turnStarted;
...
function take(px,py){
 toolMotion={x:px,y:py,start:performance.now(),tool};const point=hit(px,py);hover=point;if(!point)return false;
 const n=E.carve(state,point.p,point.dir,tool,E.TOOLS[tool].batch);
 if(n){harvest=Math.round((harvest+n)*10)/10;dirty=true;for(let i=0;i<Math.min(n,6);i++)particles.push({from:point.p,player:state.player,start:performance.now()-i*10});if(particles.length>100)particles=particles.slice(-100);if(E.harvestGrams(state,harvest)>=.001)state.turnStarted=true;audio.effect('dig',state.sediment||'sand',tool);updateUI();}return n>0;
}
function finish(){
 if(phase!=='dig')return;drag=null;pointers.clear();toolMotion=null;
...
function complete(ko){
 const actor=state.player,taken=harvest,environmentFall=anim?.environment;if(environmentFall){state.result={winner:null,reason:'environment'};}else {E.endTurn(state,taken,ko);if(state.player!==actor)state.turnStarted=false;}
...
function pass(){if(phase!=='observe'||state.result||storageBlocked||!state.turnStarted)return;checkpoint=E.encode(state);harvest=0;complete(false);}
function showResult(){
...
function newGame(seed,duration,turnDuration,stage='random',options={}){
 ...
 state=next;toolMotion=null;storageBlocked=false;if(!save()){state=old;storageBlocked=wasBlocked;throw Error('새 경기를 저장하지 못해 기존 경기를 유지했습니다.');}
...
function cancelGesture(message){toolMotion=null;if(phase==='dig'&&checkpoint){state=E.decode(checkpoint);checkpoint='';harvest=0;particles=[];phase='observe';dirty=true;if(message)toast(message);}drag=null;pointers.clear();updateUI();}
...
for(const b of document.querySelectorAll('[data-tool]')){
 const id=b.dataset.tool,sprite=W.toolSprite(id),img=b.querySelector('img');img.src=sprite.toDataURL();toolSprites[id]=sprite;cursors[id]='url("'+sprite.toDataURL()+'") 8 8, crosshair';
...
function openSetup(){if(phase!=='observe'||drag)return;$('seedInput').value=state.seed;$('duration').value=state.duration;$('durationUnit').value='1';$('turnDuration').value=state.turnDuration;$('turnUnit').value='1';$('materialPhysics').checked=state.materialPhysics!==false;$('pilePercent').value=state.pilePercent||100;$('sedimentSelect').value=state.sediment||'sand';for(const key of ['time','season','weather'])$(key+'Select').value=state.environment?.[key]||({time:'cycle',season:'cycle',weather:'auto'})[key];$('setupError').textContent='';$('setup').showModal();}
...
$('setupForm').onsubmit=e=>{e.preventDefault();try{newGame($('seedInput').value.trim(),E.seconds($('duration').value,$('durationUnit').value),E.seconds($('turnDuration').value,$('turnUnit').value),$('stageSelect').value,{materialPhysics:$('materialPhysics').checked,pilePercent:Number($('pilePercent').value),sediment:$('sedimentSelect').value,time:$('timeSelect').value,season:$('seasonSelect').value,weather:$('weatherSelect').value});$('setup').close();}catch(err){console.error('Invalid new game',err);$('setupError').textContent=err.message;}};
$('randomSeed').onclick=()=>{$('seedInput').value='SAND-'+nonce().slice(0,8);};
$('replay').onclick=()=>{try{newGame(state.seed,state.duration,state.turnDuration,$('stageSelect').value||'random',{materialPhysics:state.materialPhysics!==false,pilePercent:state.pilePercent||100,sediment:state.sediment||'sand',time:state.environment?.time||'cycle',season:state.environment?.season||'cycle',weather:state.environment?.weather||'auto'});$('result').close();}catch(e){toast(e.message);}};
...
function frame(now){
 ...
 if(active){
  ...
  if((phase==='observe'||phase==='dig')&&state.turnDuration&&state.turnStarted&&!state.result){state.turnRemaining=Math.max(0,state.turnRemaining-dt);if(state.turnRemaining<=0){if(phase==='dig')finish();else{drag=null;pointers.clear();pass();}}}
...
if(mc?.registerTool){
 const registrations=[{name:'read_sand_match',description:'Read the current local match, selected tool and flag specifications.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:true},execute:()=>({player:state.player+1,scores:state.scores.map(v=>E.grams(state,v)),phase,tool,turnStarted:!!state.turnStarted,turnRemaining:state.turnRemaining,remaining:state.remaining,harvest:E.harvestGrams(state,harvest),stage:state.stage,flag:{...state.flag},camera:{yaw,pitch,zoom},support:E.support(state).ratio,result:state.result})},
...
load();state.turnStarted??=false;setScene();audio.setMusic($('music').checked);audio.fx=$('sfx').checked;audio.setVolume(Number($('volume').value)/100);updateUI();resize();requestAnimationFrame(frame);if(state.result)showResult();
})();
